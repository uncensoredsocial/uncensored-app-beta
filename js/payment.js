...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // =========================
  // CREATE INVOICE - MULTI-ATTEMPT VERSION
  // =========================
  ...
  async function handleGenerateInvoice() {
    ...
    try {
      ...
      for (let i = 0; i < requestAttempts.length; i++) {
        try {
          const res = await safeFetch(`${PAYMENT_API_BASE_URL}/payments/create`, {
            ...
            body: JSON.stringify(requestAttempts[i])
          });
          ...
          if (res.ok) {
            ...
            // If it's a plan error, try next format
            if (lastError.toLowerCase().includes('plan') && i < requestAttempts.length - 1) {
              continue;
            }
            
            showToast(lastError, "error", 4500);
            return;
          }
        } catch (err) {
          lastError = err.message;
          
          // If network error on first try, stop
          if (i === 0 && err.message.includes('Failed to fetch')) {
            throw err;
          }
        }
      }
      ...
      // All attempts failed
      showToast("Could not create invoice. Please try again later.", "error");
