...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function handleGenerateInvoice() {
    ...
    try {
      ...
      // FIXED: Use the correct format that matches your backend
      const requestBody = {
        plan_slug: planSlug,  // Use plan_slug to match your plans table
        currency: "XMR",
        amount_crypto: Number(amountXmr.toFixed(12))
        // Don't send amount_cents - let backend get it from plans table
      };
      ...
      if (!res.ok) {
        // Try one fallback format if first fails
        if (res.status === 400 && data.error && data.error.includes("plan")) {
          // Fallback: Try with both plan and plan_slug
          const fallbackBody = {
            plan: planSlug,
            plan_slug: planSlug,
            currency: "XMR",
          ...
          const fallbackRes = await safeFetch(`${PAYMENT_API_BASE_URL}/payments/create`, {
            method: "POST",
            timeoutMs: 15000,
            headers: {
              "Content-Type": "application/json",
          ...
          if (!fallbackRes.ok) {
            throw new Error(fallbackData?.error || `HTTP ${fallbackRes.status}: ${fallbackRes.statusText}`);
          }
          
          // Process successful fallback response
          processInvoiceResponse(fallbackData, planSlug, amountXmr);
          return;
        }
      ...
      processInvoiceResponse(data, planSlug, amountXmr);
      ...
      if (err.message.includes("plan")) {
        errorMsg = "Plan not found. Please contact support.";
  ...
  // Helper function to process invoice response
  function processInvoiceResponse(data, planSlug, amountXmr) {
    ...
    // Fill UI
    if (invoicePlanLabel) {
      invoicePlanLabel.textContent = selectedPlan === "yearly" ? "Yearly Verified" : "Monthly Verified";
    }

    if (invoiceAmountLabel) {
      const cryptoAmt = (invoice.amount_crypto ?? amountXmr);
      const usdAmt = (invoice.amount_usd ?? selectedPlanUsd);
      invoiceAmountLabel.textContent = `${Number(cryptoAmt).toFixed(6)} XMR (≈ $${Number(usdAmt).toFixed(2)} USD)`;
    }
  ...
  async function testBackendDirectly() {
    ...
    try {
      console.log("Testing backend with:", testData);
      ...
      alert(`Backend response: ${res.status} ${res.statusText}\n\n${JSON.stringify(data, null, 2)}`);
    } catch (err) {
  ...
  window.testBackend = testBackendDirectly;
