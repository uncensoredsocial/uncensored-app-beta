...
// These are read from Railway .env
// Example values you will set there:
// MONERO_RPC_URL=http://127.0.0.1:18082/json_rpc  (if backend + wallet on same box)
// or MONERO_RPC_URL=https://monero.yourdomain.com/json_rpc (if proxied)
// MONERO_RPC_USERNAME=someUser
// MONERO_RPC_PASSWORD=someStrongPassword
const MONERO_RPC_URL = process.env.MONERO_RPC_URL || null;
const MONERO_RPC_USERNAME = process.env.MONERO_RPC_USERNAME || null;
const MONERO_RPC_PASSWORD = process.env.MONERO_RPC_PASSWORD || null;

if (!MONERO_RPC_URL) {
  console.warn('⚠ MONERO_RPC_URL is not set. Monero watcher will be disabled.');
}

/**
 * Simple helper to call monero-wallet-rpc JSON-RPC.
 * Uses Node 18+ built-in fetch (no node-fetch import needed).
 */
async function callMoneroRpc(method, params = {}) {
  if (!MONERO_RPC_URL) {
    throw new Error('MONERO_RPC_URL not configured');
  }
  ...
  // Basic auth if username/password set
  if (MONERO_RPC_USERNAME && MONERO_RPC_PASSWORD) {
    const auth = Buffer.from(
      `${MONERO_RPC_USERNAME}:${MONERO_RPC_PASSWORD}`
    ).toString('base64');
  ...
  if (!res.ok) {
    ...
    throw new Error(`Monero RPC HTTP error: ${res.status} - ${text}`);
  }

  const json = await res.json();
  if (json.error) {
    throw new Error(`Monero RPC error: ${json.error.message || 'Unknown error'}`);
  }
